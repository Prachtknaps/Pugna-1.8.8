...
public class TeamCommand implements CommandExecutor {
    ...
    private void execute(CommandSender sender, String[] arguments) {
        if (arguments.length == 0) {
            String invalidUsageMessage = ChatConfig.getMessage(Message.INVALID_USAGE);
            String usageMessage = ChatConfig.getMessage(Message.TEAM_USAGE);
    ...
    private List<String> getListMessages() {
        ...
        if (gameManager.getTeams().isEmpty()) {
            String message = ChatConfig.getMessage(Message.TEAM_LIST_EMPTY);
            messages.add(message);
            return messages;
        }

        String header = ChatConfig.getMessage(Message.TEAM_LIST_HEADER);
        messages.add(header);
        ...
        for (Team team : gameManager.getTeams()) {
            ...
            String message = ChatConfig.getChatMessage(Message.TEAM_LIST_ENTRY).team(team.getName()).players(members).toString();
            messages.add(message);
        }
    ...
    private void handleRename(Player player, String name) {
        ...
        if (existingTeam != null) {
            ...
            if (existingTeam.getId() != team.getId()) {
                message = ChatConfig.getMessage(Message.TEAM_RENAME_TAKEN);
            } else {
                message = ChatConfig.getChatMessage(Message.TEAM_RENAME_SAME_NAME).team(team.getName()).toString();
            }
            player.sendMessage(message);
        } else {
