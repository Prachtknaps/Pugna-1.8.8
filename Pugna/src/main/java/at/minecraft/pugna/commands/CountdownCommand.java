...
public class CountdownCommand implements CommandExecutor {
    ...
    private void execute(CommandSender sender, String[] arguments) {
        if (arguments.length != 2 || !arguments[0].equalsIgnoreCase("set")) {
            ...
            String usageMessage = ChatConfig.getMessage(Message.COUNTDOWN_USAGE);
        ...
        try {
            ...
            String message = ChatConfig.getMessage(Message.COUNTDOWN_INVALID_NUMBER);
        ...
        if (state == GameState.LOBBY_COUNTDOWN) {
            ...
            if (lobbyCountdown == null) {
                String message = ChatConfig.getMessage(Message.COUNTDOWN_NOT_ACTIVE);
                sender.sendMessage(ChatColor.stripColor(message));
                return;
            }
            ...
            String message = ChatConfig.getChatMessage(Message.COUNTDOWN_UPDATE_SUCCESS).time(time).unit(unit).toString();
            sender.sendMessage(ChatColor.stripColor(message));
            return;
        }
    ...
    private void execute(Player player, String[] arguments) {
        if (!player.isOp()) {
            String message = ChatConfig.getMessage(Message.NO_PERMISSION);
